...
Requires=docker-entrypoint.service vpnhealthcheck.service systemd-logind.service systemd-user-sessions.service
EOF
...
cat >/etc/systemd/system/vpnhealthcheck.timer <<EOF
# /etc/systemd/system/vpnhealthcheck.timer

[Unit]
Description=Run vpn health check every 5 minutes
Requires=vpnhealthcheck.service

[Timer]
Unit=vpnhealthcheck.service
OnActiveSec=5min
...
cat >/etc/systemd/system/vpnhealthcheck.service <<EOF
# /etc/systemd/system/vpnhealthcheck.service

[Unit]
Description=Cronjob for vpnhealthcheck.service

# This will auto-start the cron timer
Wants=vpnhealthcheck.timer

[Service]
EnvironmentFile=/root/healthcheck-env
Type=oneshot
ExecStart=/bin/bash -c /root/healthcheck.sh
...
systemctl enable docker-entrypoint.service vpnhealthcheck.service
